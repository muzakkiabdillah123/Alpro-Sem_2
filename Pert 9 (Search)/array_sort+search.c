...
void bubble(int type)
{
    ...
    do
    {
        ...
        for (i = 0; i <= (urutan - 1); i++)
        {
            if (type == 1)
            {
                if (data[i].akhir > data[i + 1].akhir)
                {
                    temp = data[i];
                    data[i] = data[i + 1];
                    data[i + 1] = temp;
...
void quick(int l, int r, int type)
{
    int i, j;
    mhs temp;
    i = l;
    ...
    if (type == 1)
    {
        do
        {
            while ((data[i].akhir < data[r].akhir) && (i <= j))
            {
                i = i + 1;
            }

            while ((data[j].akhir > data[l].akhir) && (i <= j))
            {
                j = j - 1;
            }
...
void sequential_search(int search_nim)
{
    int i = 0;
    int nim_cari = search_nim;
    int match = 0;

    while ((i <= urutan) && (match == 0))
    {
...
void binary_search(int awal, int akhir, int search_nim)
{
    int nim_cari = search_nim;
    int i = awal;
    int j = akhir;
...
void isian()
{
    int sort, sort_type, show_type, search_type, search_nim;
    char ulangi;
    do
    {
    ...
    if (ulangi == 'N')
    {
        do
        {
            p("\n\n\n~Pilih menu tampil data : \n");
            p("[1] Tampilkan seluruh data (Sort)\n");
            p("[2] Tampilkan salah satu data saja (Search)\n");
            p("Silahkan pilih nomor menu lalu tekan enter : ");
            s("%d", &show_type);
        } while (show_type != 1 && show_type != 2);
    }
    ...
    else
    {
        ...
        do
        {
            p("\n~ Pilih menu cari data mahasiswa : \n");
            p("[1] Sequential search\n");
            p("[2] Binary search\n");
            p("Silahkan pilih nomor menu lalu tekan enter : ");
            s("%d", &search_type);
        ...
        if (search_type == 1)
        {
            sequential_search(search_nim);
        }
        else
        {
            quick(0, urutan, sort_type);
            binary_search(0, urutan, search_nim);
        }
    }
}
